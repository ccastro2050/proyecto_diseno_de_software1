...
WORKDIR /app
ENV DOTNET_USE_POLLING_FILE_WATCHER=1 ASPNETCORE_URLS=http://0.0.0.0:8052
EXPOSE 8052
...
| `EXPOSE 8052` | **Documenta** en qué puerto escucha el programa | No abre nada: quien publica el puerto es el `ports:` del compose |
...
flowchart LR
    ...
    subgraph PC["Su PC — Docker Desktop (el 'centro de datos')"]
        subgraph RED["red interna del compose (LAN virtual, con DNS propio)"]
            APIFACTURAS["SERVIDOR DE APLICACIONES<br/>contenedor api-facturas<br/>hostname: api-facturas · escucha en 8052"]
            POSTGRES[("SERVIDOR DE BASE DE DATOS<br/>PostgreSQL · contenedor postgres<br/>hostname: postgres · escucha en 5432")]
        end
    end
    NAV -->|"localhost:8052"| APIFACTURAS
    APIFACTURAS -->|"postgres:5432 (DNS de Docker)"| POSTGRES
    CLIENTE -.->|"opcional (diagnóstico):<br/>localhost:15452"| POSTGRES
```
...
**Sin compose**, para levantar este proyecto —que tiene **2 servicios**— hay
que escribir esto, en este orden, cada vez:

```powershell
# 1. Crear la red, para que los contenedores se encuentren por su nombre
docker network create proyecto_diseno_de_software1_default

# 2. postgres
docker run -d --name postgres --network proyecto_diseno_de_software1_default --restart unless-stopped `
  ...
  -p 15452:5432 postgres:16-alpine
...
docker run -d --name api-facturas --network proyecto_diseno_de_software1_default --restart unless-stopped `
  ...
  -v /app/bin `
  -v /app/obj `
  -p 8052:8052 api-facturas

```

**4 comandos**, con sus flags, en un orden que no se puede equivocar.
...
```yaml
URL_API: http://api-facturas:8052
```
...
|---|---|---|
| **Su navegador** | `http://localhost:8052` | Está fuera de Docker. Usa el puerto **publicado** |
| **Otro contenedor** | `http://api-facturas:8052` | Están en la misma red: se llaman por el **nombre del servicio** |
...
> **En este proyecto los dos números son iguales** (`8052:8052`), y eso
...
Si alguien cambiara esa línea por `http://localhost:8052`, el front
...
```powershell
docker network inspect proyecto_diseno_de_software1_default
```
...
`docker-compose.yml` y le pega `_default`. La carpeta `proyecto_diseno_de_software1` produce
la red `proyecto_diseno_de_software1_default`. Tres formas de confirmarlo:
...
docker inspect proyecto_diseno_de_software1-api-facturas-1 --format '{{range $k,$v := .NetworkSettings.Networks}}{{$k}}{{end}}'
```
...
network proyecto_diseno_de_software1_default has active endpoints (name:"proyecto_diseno_de_software1-api-facturas-1" …)
```
...
> contenedor lo alcanza: desde `api-facturas` de este proyecto,
...
docker compose exec api-facturas curl -s -o /dev/null -w "%{http_code}\n" \
